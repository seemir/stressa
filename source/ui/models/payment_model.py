...
class PaymentModel(Model):
    ...
    def generate_payment_plan(self):
        ...
        if all(val in restructure_model.data.keys() for val in
               self.payment_keys):
            if "nedbetalingsplan_annuitet" in restructure_model.data.keys():
                payment_data_model_fixed = TableModel(
                    pd.DataFrame(
                        restructure_model.data["nedbetalingsplan_annuitet"]),
                    alignment=Qt.AlignCenter)
                self.parent.ui_form.table_view_annuitet.setModel(
                    payment_data_model_fixed)
            if "nedbetalingsplan_serie" in restructure_model.data.keys():
                payment_data_model_serie = TableModel(
                    pd.DataFrame(
                        restructure_model.data["nedbetalingsplan_serie"]),
                    alignment=Qt.AlignCenter)
                self.parent.ui_form.table_view_serie.setModel(
                    payment_data_model_serie)

            self.set_line_edits("", line_edits=self.payment_keys,
                                data=restructure_model.data)

        elif all(
                val in analysis_model.data.keys() for val in self.payment_keys):
            if "nedbetalingsplan_annuitet" in analysis_model.data.keys():
                payment_data_model_fixed = TableModel(
                    pd.DataFrame(
                        analysis_model.data["nedbetalingsplan_annuitet"]),
                    alignment=Qt.AlignCenter)
                self.parent.ui_form.table_view_annuitet.setModel(
                    payment_data_model_fixed)
