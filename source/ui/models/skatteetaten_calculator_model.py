...
class SkatteetatenCalculatorModel(Model):
    ...
    def net_wealth(self):
        ...
        net_wealth = self.calculate_net_wealth("netto_formue", verdi_bolig,
                                               bankinnskudd, gjeld)
        if net_wealth != "0" or not net_wealth:
            self.set_line_edit("netto_formue",
                               data=Money(str(net_wealth)).value())
        else:
            self.clear_line_edit("netto_formue")

    def calculate_net_wealth(self, line_edit: str, real_estate: str,
                             bank_deposit: str, debt: str):
        """
        method for calculating net wealth

        """
        net_wealth = (Decimal(
            real_estate.replace(" ", "").replace("kr", "")) if real_estate else
                      Decimal("0")) + (
                         Decimal(bank_deposit.replace(" ", "").replace("kr",
                                                                       "")) if bank_deposit
                         else Decimal("0")) - (
                         Decimal(debt.replace(" ", "").replace("kr",
                                                               "")) if debt else
                         Decimal("0"))
    ...
    def calculate_tax_income(self):
        ...
        if "brutto_inntekt_total" in self.data.keys() and "skatte_aar" in self.data.keys() and \
            ...
            tax_form = {key: val.replace(" ", "").replace("kr", "") for key, val
                        in self.data.items() if key in self.tax_input}

            brutto_inntekt = Money(
                budget_model.data["brutto_inntekt_total_aar"])

            if "trygde_inntekt_total_aar" in budget_model.data.keys():
                trygd = Money(
                    budget_model.data["trygde_inntekt_total_aar"]).value()
            else:
                trygd = "0"

            total_sub_income = Money(brutto_inntekt.value()) + Money(trygd)

            tax_form.update({'inntekt_total': total_sub_income.replace(" ",
                                                                       "").replace(
                "kr", "")})
            ...
            tax_data = SkatteetatenTaxProcessing(
                tax_data=tax_form).skatteetaten_tax_info
