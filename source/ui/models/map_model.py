...
from PyQt5.QtWebEngineWidgets import \
    QWebEngineView  # pylint: disable=no-name-in-module
...
class MapModel(Model):
    ...
    @staticmethod
    def show_map(coords: list, web_engine_view: QWebEngineView,
                 pop_up: str = None, university=None,
                 kindergarden=None, schools=None, highschools=None,
                 transport=None, charging=None,
        ...
        max_width = 400
        bytes_io = BytesIO()
        map_builder = Map(location=coords, tiles='CartoDB positron',
                          zoom_start=16)
        map_icon = CustomIcon(up(up(__file__)) + "/images/marker.png",
                              icon_size=icon_size)

        if kindergarden:
            kindergarden_cluster = MarkerCluster(name='barnehage',
                                                 show=False).add_to(map_builder)
            for pois in kindergarden:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/kindergarden.png",
        ...
        if highschools:
            ...
            for pois in highschools:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/highschools.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if university:
            ...
            for pois in university:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/university.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if transport:
            ...
            for pois in transport:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/transport.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if charging:
            ...
            for pois in charging:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/charging.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if bicycle:
            bicyle_cluster = MarkerCluster(name='bysykler', show=False).add_to(
                map_builder)
            for pois in bicycle:
        ...
        shops_cluster = MarkerCluster(name="butikker", show=False).add_to(
            map_builder)

        if groceries:
            for pois in groceries:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/groceries.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if services:
            for pois in services:
                pois_icon = CustomIcon(
                    up(up(__file__)) + "/images/services.png",
                    icon_size=small_icon)
                lat = pois["Breddegrad"]
                long = pois["Lengdegrad"]
                pois_pop_up = Popup(CreateHtmlTable(pois).html_table(),
        ...
        if pop_up:
            Marker(coords, icon=map_icon,
                   popup=Popup(pop_up, max_width=max_width)).add_to(
                map_builder)
        else:
        ...
        TileLayer('https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}{r}.png',
                  attr='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> '
                       ...
                       'France</a>', name='openstreetmap_hot',
                  show=False).add_to(map_builder)
        ...
        TileLayer('https://tileserver.memomaps.de/tilegen/{z}/{x}/{y}.png',
                  attr='Map <a href="https://memomaps.de/">memomaps.de</a> '
                       ...
                       'OpenStreetMap</a> contributors', name='openvkarte',
                  show=False).add_to(
            map_builder)

        TileLayer('http://tile.mtbmap.cz/mtbmap_tiles/{z}/{x}/{y}.png',
                  attr='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> '
                       'contributors &amp; USGS', name='mtbmap',
                  show=False).add_to(map_builder)
